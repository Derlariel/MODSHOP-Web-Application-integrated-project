  ...
  <div class="flex flex-col max-w-full sm:max-w-2xl w-full">
    ...
    <div class="flex items-center w-full">
      <div class="flex flex-wrap items-center content-center flex-1 border border-gray-300 rounded-md bg-white min-h-[42px] px-2 py-1 gap-1">
        <div v-if="selectedSizes.length === 0" class="text-gray-500 text-sm py-1">
          Storage Range
        </div>
        <div
          v-for="size in selectedSizes"
          :key="size"
          class="itbms-storage-size-item bg-blue-100 text-blue-800 text-sm rounded-full px-3 py-1 flex items-center shadow-sm border border-blue-200 transition-colors hover:bg-blue-200"
        >
          {{ formatStorageSize(size) }}
          <button
            @click="removeStorageSize(size)"
            class="itbms-storage-size-item-clear ml-2 text-blue-600 hover:text-red-600 transition-colors"
          >
            <X class="w-3 h-3" />
          </button>
        </div>
      ...
      <div class="flex-shrink-0 flex">
        <button
          ...
          class="itbms-storage-size-filter px-4 py-2 bg-gray-500 border border-gray-500 hover:bg-gray-400 transition rounded-md h-[42px] ml-2"
        >
