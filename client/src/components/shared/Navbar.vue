...
<template>
  <div class="w-full sticky top-0 z-50">
    ...
    <nav class="bg-white shadow text-gray-800">
      <div
        class="relative flex justify-between items-center h-16 w-full px-4 sm:px-6 lg:px-8"
      >
        <!-- Logo (ซ้ายสุด) -->
        <router-link to="/">
        ...
        <div class="hidden lg:flex justify-center flex-1">
          <div class="max-w-screen-xl w-full mx-auto">
            <ul
              class="flex justify-center space-x-6 lg:space-x-12 xl:space-x-24 font-semibold"
            >
              <li
                :class="
                  route.path === '/main'
        ...
        <div class="hidden lg:flex items-center space-x-4">
          ...
          <div @click="appStore.toggleAdminMode" class="cursor-pointer">
            <component
              :is="appStore.adminMode ? UserCog : User"
              class="w-5 h-5"
            />
          </div>
      ...
      <div
        ...
        class="lg:hidden px-6 py-4 space-y-4 font-medium border-t font-semibold"
      >
        <!-- Top Right Icons -->
        <div class="flex justify-end space-x-4 mb-4">
          <Search class="w-5 h-5 cursor-pointer hover:text-blue-600" />
          <Heart class="w-5 h-5 cursor-pointer hover:text-blue-600" />
          <ShoppingCart class="w-5 h-5 cursor-pointer hover:text-blue-600" />
          <div @click="appStore.toggleAdminMode" class="cursor-pointer">
            <component
        ...
        <div class="relative mb-4">
          <input
            type="text"
            placeholder="What are you looking for?"
            class="w-full pl-10 pr-4 py-2 rounded-full border border-gray-300 focus:ring-2 focus:ring-blue-400"
          />
          <Search
            class="absolute top-1/2 left-3 transform -translate-y-1/2 text-gray-400 w-5 h-5"
          />
        ...
        <!-- Menu Links -->
        <router-link to="/main" class="block hover:text-blue-600"
          >Home</router-link
        >
        <router-link to="/sale-items" class="block hover:text-blue-600"
          >Product</router-link
        >
        <router-link to="/brand" class="block hover:text-blue-600"
          >Brand</router-link
        >
